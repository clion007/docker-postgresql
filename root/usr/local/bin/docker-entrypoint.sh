...
file_env() {
	...
	local def="${2:-}"
	# indirect expansion is bash-only; emulate it with eval in POSIX sh
	if eval "[ -n \"\${$var:-}\" ]" && eval "[ -n \"\${$fileVar:-}\" ]"; then
	...
	if eval "[ -n \"\${$var:-}\" ]"; then
		eval "val=\"\${$var}\""
	elif eval "[ -n \"\${$fileVar:-}\" ]"; then
		eval "val=\"\$(cat \"\${$fileVar}\")\""
	fi
	export "$var"="$val"
	unset "$fileVar"
}
...
_is_sourced() {
	# bash's FUNCNAME array is unavailable; detect via the script name instead
...
docker_init_database_dir() {
	...
	# (process substitution is bash-only; use a temp file in POSIX sh)
	pwfile="$(mktemp)"
	printf '%s\n' "$POSTGRES_PASSWORD" > "$pwfile"
	eval 'initdb --username="$POSTGRES_USER" --pwfile="'"$pwfile"'" '"$POSTGRES_INITDB_ARGS"' "$@"'
	rm -f "$pwfile"
}
...
docker_process_init_files() {
	# (the bash original keeps a "psql" array alias for backwards compatibility; not needed in POSIX sh)
	printf '\n'
	local f
	for f; do
		case "$f" in
...
docker_process_sql() {
	# arrays are bash-only; build the command as a string and eval it
	local query_runner="psql -v ON_ERROR_STOP=1 --username \"$POSTGRES_USER\" --no-password --no-psqlrc"
	if [ -n "$POSTGRES_DB" ]; then
		query_runner="$query_runner --dbname \"$POSTGRES_DB\""
	fi
	PGHOST= PGHOSTADDR= eval "$query_runner \"\$@\""
}
...
docker_setup_env() {
	...
	# bash arrays have no POSIX equivalent; use a space-separated string instead
	: "${OLD_DATABASES:=}"
	...
	if [ -s "$PGDATA/PG_VERSION" ]; then
		...
		for d in /var/lib/postgresql /var/lib/postgresql/data /var/lib/postgresql/*/docker; do
			if [ -s "$d/PG_VERSION" ]; then
				OLD_DATABASES="$OLD_DATABASES $d"
			fi
		done
		if [ -z "$OLD_DATABASES" ] && [ "$PG_MAJOR" -ge 18 ] && {
			# in BusyBox, "mountpoint" only checks dev vs ino which will notably miss bind mounts entirely (which almost all Docker volume mounts are)
...
docker_temp_server_start() {
	...
	# (POSIX sh printf has no "%q", so quote every argument manually)
	quoted=
	for arg in "$@"; do
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		# (bash-only ${!POSTGRES_@} is not available; enumerate and unset via env instead)
		for var in $(env | awk -F= '/^POSTGRES_/ { print $1 }'); do
